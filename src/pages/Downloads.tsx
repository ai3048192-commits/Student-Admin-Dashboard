...
export default function DownloadsPage() {
  ...
  useEffect(() => {
    fetchDownloadsFromSupabase();
  }, []);

  const fetchDownloadsFromSupabase = async () => {
    setLoading(true);
    try {
      ...
      console.error("خطأ في جلب الملفات:", err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDownloadSingleFile = (fileObj: any, fileKey: string) => {
  ...
  return (
    <div className="space-y-6 sm:space-y-8  text-slate-800 min-h-screen  " dir="rtl">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-5">
          <div className="space-y-2.5">
            ...
            <p className="text-xs sm:text-sm text-blue-100 max-w-2xl leading-relaxed">
              استعرض وحمل جميع المراجع والملفات المرفوعة من قِبل الأكاديمية بنقرة واحدة بكل سهولة.
            </p>
          </div>
      ...
      {/* 2. شريط البحث المطور وفلاتر التخصصات */}
      ...
      {/* 3. شبكة الكروت (مُحسنة للموبايل وجميع الشاشات بعناصر منفصلة في أسطر مستقلة) */}
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
          {filteredDownloads.map((record) => (
            ...
            >
              ...
              {/* قسم الملفات المتاحة للتحميل (كل ملف في سطر مستقل ومنظم تماماً) */}
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto pr-0.5 scrollbar-thin">
                  {record.filesInfo && record.filesInfo.map((file: any, fIndex: number) => {
                    ...
                    return (
                      ...
                      >
                        {/* اسم الملف ونوعه في سطر أو جانب */}
                        ...
                        {/* زر التحميل في الموبايل يأخذ مساحة كاملة أو واضحة */}
                        <button
                          onClick={() => handleDownloadSingleFile(file, fileKey)}
                          disabled={isDownloading}
                          className="w-full sm:w-auto inline-flex items-center justify-center gap-1.5 px-3 py-2 bg-blue-600 hover:bg-blue-700 active:scale-95 text-white rounded-xl font-bold text-xs shadow-xs transition-all shrink-0"
                        >
                          {isDownloading ? (
                            <>
